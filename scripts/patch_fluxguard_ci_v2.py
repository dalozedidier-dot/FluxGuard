...
def patch_runs_on(text: str) -> tuple[str, int]:
    ...
    for line in text.splitlines(True):
        ...
        if m:
            out_lines.append(f"{m.group(1)}ubuntu-22.04\n")
            n += 1
        else:
...
def patch_setup_python_cache(text: str) -> tuple[str, int]:
    ...
    Conservative text patch: only handles typical indentation.
    '''
...
def main() -> int:
    ...
    ap.add_argument("--rename-blank", action="store_true", help="Rename .github/workflows/blank.yml to smoke-tests.yml if present.")
    args = ap.parse_args()
    ...
    if args.rename_blank:
        ...
        if blank.exists():
            ...
            if target.exists():
                ...
                blank.rename(target)
                print(f"Renamed: {blank.name} -> {target.name}")
